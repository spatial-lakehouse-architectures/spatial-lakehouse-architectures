...
PARTITIONED BY (
  bucket(utm_hemisphere, 2),
  bucket(utm_zone_number, 12),
  bucket(grid_100km_x, 10),
  bucket(grid_100km_y, 10)
)
USING iceberg
TBLPROPERTIES (
  'format-version' = '2',
...
df_partitioned = df.withColumn("centroid_x", col("geom").cast("geometry").ST_X()) \
                   .withColumn("centroid_y", col("geom").cast("geometry").ST_Y()) \
