...
5. **Push a partition predicate into Iceberg.** The `event_ts` range filter is pushed into Iceberg manifest planning so only the relevant data files are opened. Combined with the tile key this is the same [predicate pushdown](/spatial-partitioning-indexing-strategies/predicate-pushdown-optimization/) that keeps the scan to a small fraction of the table.
