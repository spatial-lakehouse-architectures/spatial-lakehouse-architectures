...
The method and metric definitions behind this harness — why files-scanned proxies predicate pushdown, why cold and warm runs must be separated — are laid out in the parent topic area, [Benchmarking and Selecting a Spatial Query Engine](/spatial-query-engines-compute/engine-benchmarking-selection/). Here we focus on the runnable code. You need Python 3.11+, DuckDB spatial ≥ 1.0, a reachable Trino coordinator with the Hive connector pointed at the same object store, and Apache Sedona 1.6 on Spark 3.5. The dataset is GeoParquet with WKB geometry and a bounding-box column, following the [GeoParquet encoding standards](/spatial-lakehouse-fundamentals-architecture/geoparquet-encoding-standards/), so all three engines can prune on the same statistics. Install the clients into one virtualenv:
